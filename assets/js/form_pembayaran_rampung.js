var form_pembayaran_rampung = new Vue({
    el: '#form-pembayaran-rampung-1',
    mounted: function() {
        console.info('form pembayaran rampung is created');
    },
    data: {
        id_uang_rampung: null,
        id_uang_muka: null,
        id_total_uang_muka: null,
        tgl_uang_rampung: null,
        nama_pelaksana: null,
        nama_jabatan: null,
        eselon: null,
        no_spt: null,
    ...
    methods: {
        calculate: function() {
            ...
            this.total_uang_rampung = 0
            app = this;
            pengeluaran.forEach(function(item, idx){
                item = parseFloat(item);
                console.info("item: "+ item + ", index: "+ idx);
                if (!isNaN(item)) {
                    app.total_uang_rampung += item;
                }
            });
            //this.total_total_uang_muka = pengeluaran.reduce((a, b) => parseFloat(a) + parseFloat(b), 0);
        },
    ...
    watch: {
        lama_pelaksanaan: function(val) {
            var id_sppd = $('#id_sppd_rampung').val();
        ...
        total_uang_rampung: function(val) {
            if (val) {
                this.kurang_lebih = parseFloat(this.total_uang_rampung) - parseFloat(this.total_uang_muka);
            }
        }
